# Generated by Powerlevel10k configuration wizard on 2024-10-30 at 21:54 IST.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    # poetry                  # poetry version running/not running etc
    # swap                  # used swap
  ...
  function prompt_poetry() {
    local POETRY_ENV="$(poetry env list &> /dev/null | grep -e Activated | sed 's/ .*//' | head -1)"
    if [ ! -z ${POETRY_ENV} ] && [ -z ${POETRY_ACTIVE} ]; then
      p10k segment -b 1 -f 010 -i '⭐' -t "poetry inactive"
    elif [ ! -z ${POETRY_ENV} ] && [ ! -z $POETRY_ACTIVE ]; then
      p10k segment -b 2 -f 011 -i '⭐' -t "$POETRY_ENV active"
    elif [ -z ${POETRY_ENV} ] && [ ! -z $POETRY_ACTIVE ]; then
      # if your shell is active and you are outside of the folder.
      p10k segment -b 3 -f 014 -i '⭐' -t "POETRY is still running"
    else 
      p10k segment -b 1 -f 009 -i '⭐' -t "POETRY absent"
    fi
  }
